...
const { handleIncomingMessageWhatsapp, sendMessageWhatsapp } = require('./whatsapp');
const app = express();
      ...
      body.entry.forEach(entry => {
          entry.changes.forEach(change => {
              if (change.value.messages) {
                  change.value.messages.forEach(event => {
                      console.log(`Received message: ${event.text.body}`);
                      handleIncomingMessage(event);
                  });
              }
          });
